...
Anti-pattern: regex-based code modification when an AST is available. Regex codemods break on any syntactic variation (newlines, comments, whitespace, alternate member access). Read `reference/grammar/ast-transforms.md` for roundtrip-safe transform patterns (recast, jscodeshift with full-fidelity nodes) and codemod catalogs.
